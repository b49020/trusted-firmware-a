 ...
 *
 * Faithful replica of the Qualcomm downstream CPUSS POR sysini (label
 * l_NORDV2_spi_cfg_start) in the Wildcat CPUSS sysini library.
 *
 ...
 * already enable forwarding, so NO global-enable / reset-release write exists
 * downstream or is needed. Our open BL31 was missing ONLY this per-SPI type
 * programming - the full Hoya CPUSS sysini was stubbed to no-ops because parts
 * hang on Oryon - so level SPIs (UFS 265/INTID297, geni 615, RSC 61) were left
 * mis-typed at reset and never delivered/cleared to the GIC.
 *
 * The HW registers are 64-bit (stride 0x8); the downstream array loop writes
 * them as 32-bit stores at +4 stride, filling consecutive low/high halves. We
 * replicate that byte-for-byte.
 *
 * Downstream macro semantics (sysini_common.spp):
 *   BIS_REG_ARRAY dst,src,#N : for i<N: mmio_write_32(dst+4*i,  src[i])  // verbatim
 *   BIC_REG_ARRAY dst,src,#N : for i<N: mmio_write_32(dst+4*i, ~src[i])  // inverted
 *
