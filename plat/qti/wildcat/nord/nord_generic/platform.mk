...
# QCCE-326 (see lib/cpus/aarch64/oryon.S) is gated to Oryon-1 MIDR variant/
# revision r3p0/r4p0/r3p4/r4p4 (Pakala); Nord's silicon reports variant 5
# (0x515F0014) and is not affected. Left disabled explicitly rather than
# relying on the errata framework's default-zero so the intent is visible
# here alongside the other Nord CPU config.
ERRATA_ORYON_QCCE_326			:=	0
...
# Use the native (wildcat/Nord) PSCI CPU bringup (qti_pm_wildcat.c),
# mirroring the hoya QTI_PM_NATIVE path.
QTI_PM_NATIVE				:=	1
...
# PSCI CPU_ON (see plat_qti_pwr_domain_on() in qti_pm_wildcat.c).
include drivers/qti/cpucp/cpucp.mk

# SMMU configuration.
include drivers/qti/smmu/smmu.mk

# PDC (Power Domain Controller) configuration. Nord's PDC has no TCS/wake-
# resource path (PDC_ENABLE_TCS=0 in nord/pdc_config.mk); low-power-mode entry
# is driven by a branch mask instead, so cmd_db/pwr_utils are not needed here.
