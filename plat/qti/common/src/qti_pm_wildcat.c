 ...
 * qti_pwr_domain_on uses the raw per-cluster NCC_ARCH register sequence (QC
 ...
 * in EL3, so qti_pwr_domain_on_finish() does not call cpucp_clkdom_init()
...
static void nord_lpm_seq_init(void)
{
	 ...
	 * per-core warm-boot vectoring in qti_pwr_domain_on starts from a known
	 * state.
	 */
	for (cl = 0U; cl < 3U; cl++) {
 ...
 */
static void nord_ncc_power_on_core(uintptr_t base, unsigned int core)
{
...
/*
 * qti_pwr_domain_on - power on a secondary Oryon core using the raw NCC_ARCH
...
/*
 * qti_pwr_domain_on_finish - per-core setup once it has come online, run on the
