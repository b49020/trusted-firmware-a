 ...
 * Oryon core on WFI it resumes at CPUCP PBL is reset vector (not BL31) and is
 * effectively lost, which is the root cause of the intermittent boot and the
 * ~2s post-shell CPU shutdown. Program every cluster is NCC_RVBAR with the BL31
 * warm-boot entrypoint so collapsed cores re-enter BL31.
 *
 * Per-cluster NCC ARCH register block (downstream tzbsp_target.h):
