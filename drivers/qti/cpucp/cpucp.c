 ...
 * SCMI Clock Management protocol over the CPUCP shared-memory transport. The
...
/*
 * write_output_payload_word - Append the SCMI response payload with 32-bit
 * @data.
 */
static void write_output_payload_word(struct scmi_packet *packet, uint32_t data)
{
...
/*
 * get_input_payload_word_at_offset - Return 32-bit data that resides in the
 * input payload at a word offset of @offset.
 */
static uint32_t get_input_payload_word_at_offset(struct scmi_packet *packet,
						 int offset)
{
...
/*
 * scmi_packet_init_payload - Initialize the SCMI payload before data is written
 * to it.
 */
static void scmi_packet_init_payload(struct scmi_packet *packet)
{
	packet->len = 4;
}
			   ...
			   unsigned int *rx_payload_size)
{
	...
	if (tx_payload_size > (CPUCP_SECIPC_RAM_LENGTH - 0x1cU)) {
		return CPUCP_SCMI_SEND_ERR_TX_SIZE;
	}

	tx_payload_words = (tx_payload_size + 3U) / 4U;
...
void cpucp_clkdom_init(void)
{
	...
	uint32_t linear_id = 1U << plat_my_core_pos();
...
/*
 * SCMI Power Domain Management POWER_STATE_SET command payload. Matches the
 * downstream Nord CPUCP host driver (ssg/cpusys/cpucp/host_v2/init/common/
 * scmi_power.c cpucp_cluster_core_power_on/_off()): domain_id is
 ...
 * in qti_pm_wildcat.c: core = (mpidr >> 8) & 0xff, cluster =
 * (mpidr >> 16) & 0xff).
 */
 ...
 *
 * Nord's native PSCI CPU_ON path (qti_pwr_domain_on() in qti_pm_wildcat.c)
 * drives the NCC_ARCH cluster registers directly and, unlike every downstream
 * reference PSCI implementation (core/power/psci/src/asic/<target>/
 * asic_nodes.c, which calls this exact SCMI command on every CPU_ON/CPU_OFF),
 * never notifies CPUCP of the change. Sending it keeps CPUCP's own view of
 * per-core power state in sync with the cores TF-A actually brings up.
 */
